...
[ Rust Core & autogram-core Engine ]
  ...
  ├─ Telegram Album Engine (Max 10 items/album + forced-single isolation)
...
4. **Telegram Album Chunking & Fallback Engine**: Membagi unggahan album menjadi kelompok maksimal 10 item (`chunk_size <= 10`) sesuai batas API Telegram. Video native tanpa audio diisolasi sebagai pesan tunggal (`item_forced_single`) agar tidak memicu `MEDIA_EMPTY`; engine mencatat RPC error, menjalankan fallback terkontrol, mengalokasikan `reply_to` topik, dan memakai *history recovery engine* (`try_recover_album_from_history`) bila terjadi gangguan.
