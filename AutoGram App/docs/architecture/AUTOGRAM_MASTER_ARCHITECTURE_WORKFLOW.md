...
2. **Telegram Album Chunking Engine**: Batas kontrak album adalah **maksimal 10 item per album** (`chunk_size <= 10`), sesuai `messages.sendMultiMedia`. Video native tanpa audio dipisahkan sebagai single (`item_forced_single`) untuk mencegah `MEDIA_EMPTY`; pipeline tetap menyediakan *Universal Forum Topic Album Routing*, *Explicit `reply_to` Allocation*, *Automatic Single Fallback Retry Engine*, dan *Partial Album History Recovery* (`try_recover_album_from_history`).
...
- **Alur Kerja Utama (v2.8.7)**:
  ...
  3. Untuk pengiriman album (*Group as Album*), `studio_orch.rs` mengeksekusi **Telegram Album Chunking Engine**, membagi item menjadi batch maksimal **10 item per album** (`chunk_size <= 10`). Video native tanpa audio dikirim single untuk mencegah `MEDIA_EMPTY`, sementara item lain tetap dikelompokkan dan setiap keputusan dicatat di transfer log.
...
| Telegram Album Chunking Engine (≤10 items + forced-single isolation) | **AKTIF** | `studio_orch.rs`, `album.rs` |
