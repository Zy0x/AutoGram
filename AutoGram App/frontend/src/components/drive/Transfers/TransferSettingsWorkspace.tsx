...
export function TransferSettingsWorkspace({
  ...
  const runPluginTest = async () => {
    const urlToTest = pluginTestUrl.trim() || 'https://www.youtube.com/watch?v=LXb3EKWsInQ';
    setPluginTestRunning(true);
    setPluginTestResult(null);
    try {
      const text = await invoke<string>('ytdlp_resolve', {
        url: urlToTest,
        autoUpdate: draft.ytdlpAutoUpdate !== false,
        checkIntervalHours: draft.ytdlpCheckIntervalHours || 6,
      ...
      const videoFmts = formats.filter((f: any) => f.vcodec && f.vcodec !== 'none');
      const audioFmts = formats.filter((f: any) => (!f.vcodec || f.vcodec === 'none') && f.acodec && f.acodec !== 'none');
      const maxRes = videoFmts.length > 0 ? (videoFmts[videoFmts.length - 1]?.resolution || videoFmts[videoFmts.length - 1]?.format_note || 'HD') : 'N/A';
      setPluginTestResult({
        success: true,
        message: t('drive_tools.plugin_test_success', {
          videos: videoFmts.length,
          audios: audioFmts.length,
          maxRes,
        ...
        details: {
          ...
          videoCount: videoFmts.length,
          audioCount: audioFmts.length,
      ...
      setPluginTestResult({
        success: false,
        message: t('drive_tools.plugin_test_failed', { error: err instanceof Error ? err.message : String(err) }),
      });
    } finally {
      setPluginTestRunning(false);
    }
  };
            ...
            <div className="td-plugin-overview-grid">
              ...
              <div className="td-settings-card td-plugin-card">
                ...
                <div className="td-plugin-status-box">
                  ...
                  <div className="td-plugin-status-runtime-row" style={{ flexWrap: 'wrap', gap: '6px' }}>
                    ...
                    <strong style={{ color: ffmpegStatus?.installed ? '#4ade80' : '#f59e0b' }}>
                      {ffmpegBusy ? t('drive_tools.plugin_ffmpeg_btn_updating') : ffmpegStatus?.installed
                        ? t('drive_tools.plugin_ffmpeg_status_installed', { version: ffmpegStatus.version || t('drive_tools.plugin_status_ready') })
                        : t('drive_tools.plugin_ffmpeg_status_not_found')}
                    </strong>
                    {ffmpegStatus?.source && ffmpegStatus.source !== 'none' && (
                      <span className="td-chip-btn" style={{ padding: '1px 6px', fontSize: '0.66rem', cursor: 'default', opacity: 0.85 }}>
                        {ffmpegStatus.source === 'app_data' ? t('drive_tools.plugin_ffmpeg_source_app_data') : ffmpegStatus.source === 'workspace_plugin' ? t('drive_tools.plugin_ffmpeg_source_workspace') : ffmpegStatus.source === 'custom' ? t('drive_tools.plugin_ffmpeg_source_custom') : t('drive_tools.plugin_ffmpeg_source_system')}
                      </span>
                    )}
                  </div>
                  ...
                  {ffmpegStatus?.installed && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '4px' }}>
                      {ffmpegStatus.supportsHttp && (
                        <span className="td-chip-btn td-chip-primary" style={{ padding: '1px 6px', fontSize: '0.68rem', cursor: 'default' }}>
                          {t('drive_tools.plugin_ffmpeg_cap_http')}
                        </span>
                      )}
                      {ffmpegStatus.av1Decoder && (
                        <span className="td-chip-btn" style={{ padding: '1px 6px', fontSize: '0.68rem', cursor: 'default', background: 'rgba(168, 85, 247, 0.15)', color: '#c084fc', borderColor: 'rgba(168, 85, 247, 0.3)' }}>
                          {t('drive_tools.plugin_ffmpeg_cap_av1', { codec: ffmpegStatus.av1Decoder })}
                        </span>
                      )}
                      {ffmpegStatus.supportsNvenc && (
                        <span className="td-chip-btn" style={{ padding: '1px 6px', fontSize: '0.68rem', cursor: 'default', background: 'rgba(16, 185, 129, 0.15)', color: '#34d399', borderColor: 'rgba(16, 185, 129, 0.3)' }}>
                          {t('drive_tools.plugin_ffmpeg_cap_nvenc')}
                        </span>
                      )}
                      {ffmpegStatus.ffprobeExecutable && (
                        <span className="td-chip-btn" style={{ padding: '1px 6px', fontSize: '0.68rem', cursor: 'default', background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa', borderColor: 'rgba(59, 130, 246, 0.3)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', paddingTop: '4px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.72rem', color: '#64748b' }}>
                      {t('drive_tools.ytdlp_auto_update_desc')}
                    </div>
                  </div>
                  <ToggleSwitch
                    checked={draft.ytdlpAutoUpdate !== false}
              ...
              <div className="td-ytdlp-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '0.78rem', fontWeight: 600, color: '#cbd5e1' }}>
                    {t('drive_tools.plugin_cookies_mode_title')}
                  </label>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
                    {(['none', 'browser', 'file'] as const).map((mode) => {
                      const active = (draft.ytdlpCookiesMode || 'none') === mode;
                      return (
                        <button
                          key={mode}
                          type="button"
                        ...
                        >
                          ...
                          {mode === 'browser' && t('drive_tools.plugin_cookies_mode_browser')}
                          {mode === 'file' && t('drive_tools.plugin_cookies_mode_file')}
                        </button>
                      );
                    })}
                  </div>
              ...
              <div className="td-ytdlp-card" style={{ gridColumn: '1 / -1' }}>
                <div className="td-ytdlp-card-header">
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '0.75rem', color: '#64748b' }}>
                      {t('drive_tools.plugin_section_test_desc')}
                    </span>
                  </div>
                </div>
