...
export function DriveFileInfoModal({ file, locationName, pathId, onClose }: Props) {
  ...
  const delivery = file.as_document
    ? t('drive.media_info_delivery_document')
    : t('drive.media_info_delivery_media');
  const createdAt = formatCreatedAt(
    file.created_at,
    i18n.language,
    t('drive.media_info_unknown'),
  );
  ...
  const importantRows: Array<[string, string]> = [
    [t('drive.media_info_name'), driveFileDisplayName(file)],
    [t('drive.media_info_size'), formatDriveBytes(file.size || 0)],
    [t('drive.media_info_mime'), file.mime_type || t('drive.media_info_unknown')],
    [t('drive.media_info_delivery'), String(delivery)],
    [t('drive.media_info_category'), file.telegram_category || file.telegramCategory || file.icon_type || t('drive.media_info_unknown')],
    [t('drive.media_info_date'), createdAt],
    ...(duration > 0 ? [[t('drive.media_info_duration'), `${duration.toFixed(duration % 1 ? 1 : 0)} s`] as [string, string]] : []),
    [t('drive.media_info_message_id'), String(file.id)],
    [t('drive.media_info_peer_id'), String(file.peer_id || file.folder_id || 'me')],
    ...(file.topic_id != null ? [[t('drive.media_info_topic_id'), String(file.topic_id)] as [string, string]] : []),
    ...(file.grouped_id != null ? [[t('drive.media_info_album_id'), String(file.grouped_id)] as [string, string]] : []),
  ];
      ...
      <section className="drive-file-info-dialog" role="dialog" aria-modal="true" aria-labelledby="drive-file-info-title">
        <header className="drive-file-info-header">
          <div className="drive-file-info-heading">
            ...
            <div>
              <h2 id="drive-file-info-title">{t('drive.media_info_title')}</h2>
              <p>{locationName || t('drive.media_info_location_unknown')}</p>
            </div>
          </div>
          <button type="button" className="drive-file-info-close" onClick={onClose} aria-label={t('drive.media_info_close')}>
            <X size={19} />
          </button>
        </header>
        ...
        <div className="drive-file-info-body">
            ...
            <div className="drive-file-info-identities">
              {pathId && (
                <button type="button" onClick={() => void copy(pathId, 'path')}>
                  <span><Link2 size={15} />{t('drive.media_info_path_id')}</span>
                  <code>{pathId}</code>
              ...
              {telegramUrl && (
                <button type="button" onClick={() => void openUrl(telegramUrl)}>
                  <span><ExternalLink size={15} />{t('drive.media_info_telegram_link')}</span>
          ...
          {links.length > 0 && (
            <div className="drive-file-info-links">
              <h3>{t('drive.media_info_embedded_links', { count: links.length })}</h3>
              {links.map((url, index) => (
          ...
          <button type="button" className="drive-file-info-expand" onClick={() => setExpanded((value) => !value)}>
            ...
            {expanded ? t('drive.media_info_less') : t('drive.media_info_more')}
          </button>
          {expanded && (
