...
function parseTelegramMarkdown(text: string) {
  ...
  let html = text
    ...
    .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" style="color: var(--accent-primary, #58a6ff); text-decoration: none;">$1</a>')
    .replace(/`(.*?)`/g, '<code style="color: var(--text-primary); font-family: monospace; background: var(--bg-hover, rgba(0,0,0,0.3)); padding: 2px 4px; border-radius: 4px;">$1</code>')
    .replace(/\n/g, '<br/>');
  return html;
}
    ...
    <div className="modal-overlay" onClick={onClose} style={{ zIndex: 9999 }}>
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '900px', width: '95%' }}>
        ...
        <div className="modal-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <div style={{ display: 'flex', background: 'var(--bg-hover)', borderRadius: '8px', padding: '4px' }}>
               <button onClick={() => setViewMode('edit')} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', background: viewMode === 'edit' ? 'var(--primary)' : 'transparent', color: viewMode === 'edit' ? 'var(--text-on-accent, #fff)' : 'var(--text-muted)', cursor: 'pointer', fontSize: '0.85rem', transition: 'all 0.2s' }}>{t('ui.generated.edit_5301648')}</button>
               <button onClick={() => setViewMode('split')} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', background: viewMode === 'split' ? 'var(--primary)' : 'transparent', color: viewMode === 'split' ? 'var(--text-on-accent, #fff)' : 'var(--text-muted)', cursor: 'pointer', fontSize: '0.85rem', transition: 'all 0.2s' }}>{t('ui.generated.split_fc8230b')}</button>
               <button onClick={() => setViewMode('preview')} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', background: viewMode === 'preview' ? 'var(--primary)' : 'transparent', color: viewMode === 'preview' ? 'var(--text-on-accent, #fff)' : 'var(--text-muted)', cursor: 'pointer', fontSize: '0.85rem', transition: 'all 0.2s' }}>{t('drive.tooltip_preview')}</button>
            </div>
        ...
        <div className="modal-body" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', fontSize: '0.85rem', color: 'var(--text-muted)', background: 'var(--bg-hover)', padding: '12px 16px', borderRadius: '8px', border: '1px solid var(--border)', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div><span style={{color: 'var(--accent-primary, #58a6ff)'}}>{t('ui.generated.link_https_url_com_f40a78d')}</span></div>
            ...
            {(viewMode === 'split' || viewMode === 'edit') && (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <textarea
                  ...
                  style={{ flex: 1, resize: 'none', fontFamily: 'monospace', padding: '16px', lineHeight: '1.5', fontSize: '0.9rem', border: '1px solid var(--border)' }}
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: '12px', background: 'var(--bg-modal)' }}>
