 ...
 * Lists central directory without extracting the whole archive;
 * opens one entry at a time on demand.
 */
...
function basenamesAt(entries: ZipEntry[], cwd: string): {
...
function iconForFile(name: string) {
  ...
  if (/\.(txt|md|json|csv|log|xml|ya?ml|html?|css|js|ts|py)$/i.test(n)) return FileText;
  return File;
}
...
export function DriveZipBrowser({ creds, messageId, folderId, archiveName }: Props) {
  ...
  const { folders, files } = useMemo(() => basenamesAt(entries, cwd), [entries, cwd]);
      ...
      <div className="drive-zip-split">
        <ul className="drive-zip-list" role="list">
          ...
          {files.map((f) => {
            ...
            const label = entryLabel(f.name, cwd);
          ...
          {folders.length === 0 && files.length === 0 && (
            <li className="drive-zip-empty">Folder kosong dalam arsip</li>
          )}
        </ul>
        ...
        <div className="drive-zip-preview-pane">
          ...
          {(preview?.kind === 'binary' || preview?.kind === 'meta') && (
            <div className="drive-zip-preview-empty">
              ...
              {preview.dataUrl && (
                <a className="td-btn-primary" href={preview.dataUrl} download={entryLabel(preview.entry, cwd)}>
                  Unduh entri
                </a>
              )}
            </div>
