...
/** Sidebar & topbar set this while pointer hovers a drop row during internal drag. */
export function setLastHoverDropKey(key: string | null): void {
  if (lastHoverDropKey === key) return;
 ...
 */
function pointInScrollClips(node: HTMLElement, clientX: number, clientY: number, pad = 14): boolean {
...
function bestKeyByGeometry(
  ...
  let best: { key: string; dist: number; strict: boolean; area: number } | null = null;
  for (const node of nodes) {
    ...
    // Distance to rectangle boundary (0 when strictly inside rect)
    ...
    if (
      ...
      (strict === best.strict && dist === best.dist && area < best.area)
    ) {
      best = { key, dist, strict, area };
    }
  }
...
export function parseDropKey(
  ...
  if (kind === 'topic') {
    if (idPart === 'all' || idPart === 'null') return { kind: 'topic', id: null };
    const tid = Number(idPart);
    return { kind: 'topic', id: Number.isFinite(tid) ? tid : null };
  }
