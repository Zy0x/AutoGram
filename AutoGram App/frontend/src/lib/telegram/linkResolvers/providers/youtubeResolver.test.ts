...
describe('youtubeResolver', () => {
  ...
  it('correctly maps manual and auto-generated subtitle tracks with human-readable language names and formats', () => {
    ...
    processYtDlpData({
      ...
      subtitles: {
        id: [
          { ext: 'vtt', url: 'https://www.youtube.com/api/timedtext?lang=id&fmt=vtt' },
          { ext: 'srt', url: 'https://www.youtube.com/api/timedtext?lang=id&fmt=srv3' },
        ],
        en: [
      ...
      automatic_captions: {
        ja: [
          { ext: 'vtt', url: 'https://www.youtube.com/api/timedtext?lang=ja&kind=asr&fmt=vtt' },
        ],
      },
    }, formats, subtitles, rawStreams);

    // Subtitle items populated
    expect(subtitles.length).toBe(3);
    expect(subtitles.some((s) => s.languageCode === 'id' && s.languageName.includes('Indonesian'))).toBe(true);
    expect(subtitles.some((s) => s.languageCode === 'en' && s.languageName.includes('English'))).toBe(true);
    expect(subtitles.some((s) => s.languageCode === 'ja' && s.languageName.includes('Japanese') && s.isAutoGenerated)).toBe(true);

    // Formats list includes downloadable subtitle entries
    const subFormats = formats.filter((f) => f.isSubtitle);
    expect(subFormats.length).toBe(4);
    expect(subFormats.some((f) => f.ext === 'srt' && f.resolution === 'ID')).toBe(true);
