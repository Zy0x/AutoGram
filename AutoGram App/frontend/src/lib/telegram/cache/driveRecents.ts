 ...
 */
import { truncateMiddle } from '../interaction/drivePower';

export type DriveRecentKind = 'saved' | 'drive' | 'chat';

/**
 * chatType: raw Telegram entity type from DriveChat.type.
 * isForum: true when the group has forum/topics enabled (Groups - Forum).
 */
export type DriveRecent = {
  ...
  label: string;
  at: number;
  chatType?: string;   // 'user' | 'group' | 'channel' | 'bot' | 'unknown'
  isForum?: boolean;  // true = Groups - Forum
};
...
export type TogglePinResult = {
  ...
  replacedItem?: DriveRecent;
};

export function toggleDrivePinResult(
  session: string,
  entry: Omit<DriveRecent, 'at'> & { at?: number },
  store?: StorageLike | null
): TogglePinResult {
  const s = store ?? storage();
  if (!s || !String(session || '').trim()) return { pins: [], action: 'unpinned' };
  ...
  if (prev.some((p) => keyOf(p) === k)) {
    const updated = savePins(session, prev.filter((p) => keyOf(p) !== k), s);
    return { pins: updated, action: 'unpinned' };
  }
  let replacedItem: DriveRecent | undefined = undefined;
  let newList = [...prev, next];
  if (newList.length > MAX_PINS) {
    replacedItem = newList.shift(); // FIFO rotate: remove oldest item
  }
  const updated = savePins(session, newList, s);
  return { pins: updated, action: 'pinned', replacedItem };
}
...
export function toggleDrivePin(
  ...
  return toggleDrivePinResult(session, entry, store).pins;
}
...
function normalizeList(raw: unknown, max: number): DriveRecent[] {
  ...
  return raw
    ...
    .map((x) => ({
      ...
      at: Number(x.at) || 0,
      chatType: typeof x.chatType === 'string' ? x.chatType : undefined,
      isForum: typeof x.isForum === 'boolean' ? x.isForum : undefined,
    }))
...
export function loadDrivePeer(
  ...
  try {
    ...
    if (raw) {
      ...
      if (p.kind === 'drive' && typeof p.id === 'number') {
        return { kind: 'drive', id: p.id };
      }
      // Drives workspace cold entry should land in Drives/Saved Messages, not a standard Telegram chat.
      if (p.kind === 'chat') {
