...
function buildProfile(tier: PerfTier): DrivePerfProfile {
  ...
  if (tier === 'low') {
    return {
      ...
      avatarBatch: 4,
      avatarQueueMax: 16,
      avatarBootPauseMs: 1200,
      folderScanDelayMs: 2500,
      thumbSoftFailMs: 60_000,
      fastNet: false,
  ...
  if (tier === 'mid') {
    return {
      ...
      avatarBatch: 8,
      avatarQueueMax: 32,
      avatarBootPauseMs: 450,
      folderScanDelayMs: 500,
      thumbSoftFailMs: 75_000,
      fastNet,
      label: 'Mode Seimbang',
    };
  }
  ...
  return {
    ...
    loadMorePage: turbo ? 140 : 100,
    chatPage: turbo ? 80 : 56,
    chatSoftPrefetchMax: turbo ? 400 : 250,
    defaultThumbQuality: 'balanced',
    defaultGridZoom: 2,
    statsDelayMs: 350,
    fullMediaStats: true,
    thumbBatch: turbo ? 24 : 16,
    thumbFlushMs: turbo ? 16 : 24,
    thumbQueueMax: turbo ? 160 : 96,
    thumbConcurrent: turbo ? 4 : 3,
    thumbResumeMs: turbo ? 60 : 120,
    thumbPrefetchRows: turbo ? 3 : 2,
    prefetchNextPage: true,
    avatarBatch: turbo ? 14 : 10,
    avatarQueueMax: turbo ? 56 : 40,
    avatarBootPauseMs: turbo ? 80 : 160,
    folderScanDelayMs: 0,
    thumbSoftFailMs: 90_000,
    fastNet: turbo,
