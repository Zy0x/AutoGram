  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-950/90 border border-indigo-500/40 text-slate-100 px-5 py-3 rounded-2xl shadow-2xl backdrop-blur-xl flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-200">
      <div className="flex items-center gap-2 border-r border-slate-800 pr-4">
        ...
        <span className="text-xs font-bold text-slate-100 font-mono">{selectedCount} {t('speedtest.filter_all')}</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        {onBatchDownload && (
          ...
          >
            <Download size={14} /> {t('speedtest.download_batch')}
          </button>
        )}
        ...
        {onBatchDelete && (
          ...
          >
            <Trash2 size={14} /> {t('speedtest.delete_batch_confirm')}
          </button>
        )}
        ...
        <button
          ...
          title={t('speedtest.clear_selection')}
        >
          <XSquare size={16} />
        </button>
      </div>
