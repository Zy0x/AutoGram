...
pub fn get_format_capability(mime_type: Option<&str>, file_name: &str) -> PreviewCapability {
    ...
    if mime.starts_with("image/") || name.ends_with(".jpg") || name.ends_with(".jpeg") || name.ends_with(".png") || name.ends_with(".webp") {
        PreviewCapability::PartialImageDecode
    } else if mime.starts_with("video/") || name.ends_with(".mp4") || name.ends_with(".mov") || name.ends_with(".mkv") || name.ends_with(".webm") {
