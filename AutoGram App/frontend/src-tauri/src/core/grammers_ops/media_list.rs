...
pub fn tl_message_to_row(
    ...
    if let Some(ref media) = m.media {
        ...
        match media {
            ...
            grammers_client::tl::enums::MessageMedia::WebPage(wp_media) => {
                let (url, title, _desc) = match &wp_media.webpage {
                    grammers_client::tl::enums::WebPage::Page(page) => {
                        (page.url.clone(), page.title.clone(), page.description.clone())
                    }
                    grammers_client::tl::enums::WebPage::Empty(e) => (format!("url_{}", e.id), None, None),
                    grammers_client::tl::enums::WebPage::Pending(p) => (p.url.clone().unwrap_or_else(|| format!("url_{}", p.id)), None, None),
                    grammers_client::tl::enums::WebPage::NotModified(_) => ("webpage".to_string(), None, None),
                };
                ...
                Some(MediaFileRow {
                    ...
                    identity_source: Some("telegram_search".into()),
                    peer_id: folder_id
                        .map(|fid| if fid == 0 { "me".into() } else { fid.to_string() })
                        .or_else(|| Some("me".into())),
                    account_id: None,
                    peer_kind: None,
                    peer_username: None,
    ...
    } else if caption.contains("http://") || caption.contains("https://") || caption.contains("t.me/") {
        let first_line = caption.lines().next().unwrap_or("link").trim();
        let name = if first_line.len() > 100 { &first_line[..100] } else { first_line }.to_string();
        let cls = crate::core::media_classifier::classify_media_item(
            &name,
            Some("text/x-url"),
            false,
