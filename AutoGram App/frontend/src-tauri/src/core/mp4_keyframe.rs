...
fn parse_moov_internal(data: &[u8]) -> Option<Vec<KeyframeEntry>> {
    ...
    for (trak_start, trak_size) in traks {
        ...
        let Some(&(mdia_off, mdia_sz)) = trak_boxes.get("mdia") else { continue; };
        ...
        // Parse timescale
        let Some(&(mdhd_off, mdhd_sz)) = mdia_boxes.get("mdhd") else { continue; };
        if mdhd_sz < 16 || mdhd_off + mdhd_sz > data.len() { continue; }
        let version = data[mdhd_off];
        let timescale = if version == 1 {
            if mdhd_sz < 24 { continue; }
            u32::from_be_bytes([data[mdhd_off + 20], data[mdhd_off + 21], data[mdhd_off + 22], data[mdhd_off + 23]])
        } else {
            u32::from_be_bytes([data[mdhd_off + 12], data[mdhd_off + 13], data[mdhd_off + 14], data[mdhd_off + 15]])
        };

        if timescale == 0 { continue; }

        let Some(&(minf_off, minf_sz)) = mdia_boxes.get("minf") else { continue; };
        let minf_boxes = read_boxes(data, minf_off, minf_off + minf_sz);
        let Some(&(stbl_off, stbl_sz)) = minf_boxes.get("stbl") else { continue; };
        let stbl_boxes = read_boxes(data, stbl_off, stbl_off + stbl_sz);

        // Extract stss (sync samples / keyframes)
        let Some(&(stss_off, stss_sz)) = stbl_boxes.get("stss") else { continue; };
        if stss_sz < 8 || stss_off + stss_sz > data.len() { continue; }
        let count = u32::from_be_bytes([data[stss_off + 4], data[stss_off + 5], data[stss_off + 6], data[stss_off + 7]]) as usize;

        let mut keyframes = Vec::with_capacity(count);
        let mut sample_ptr = stss_off + 8;

        for i in 0..count {
            if sample_ptr + 4 > data.len() { break; }
            let sample_idx = u32::from_be_bytes([data[sample_ptr], data[sample_ptr + 1], data[sample_ptr + 2], data[sample_ptr + 3]]);
            sample_ptr += 4;
