...
pub fn ffmpeg_supports_av1(ff_exe: &Path) -> bool {
    ...
    probe_ffmpeg_capabilities(ff_exe)
        .map(|c| c.av1_decoder.is_some())
        .unwrap_or(false)
}
...
fn get_static_fallback_jpeg() -> Vec<u8> {
    vec![
        ...
        0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0xff, 0xc0, 0x00, 0x11, 0x08, 0x01,
        0x0e, 0x01, 0xe0, 0x03, 0x01, 0x22, 0x00, 0x02, 0x11, 0x01, 0x03, 0x11, 0x01, 0xff, 0xc4,
        ...
        0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
        ...
        0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3F, 0x00, 0xF9, 0xFE, 0x8A, 0x28, 0x00, 0x00, 0xFF,
        0xD9,
    ]
}
...
pub fn extract_ffmpeg_frame_sync(
    sample_bytes: &[u8],
    quality: &str,
    ext_hint: &str,
) -> Option<Vec<u8>> {
        ...
        if result.is_none() {
            if let Ok(out) = status4 {
                ...
                tg_log::warn(
                    ...
                    "ffmpeg_frame_failed",
                    &format!(
                        "size={} ext={ext} av1={is_av1} err1='{err1}' err2='{err2}'",
                        sample_bytes.len()
                    ),
                );
            }
                ...
                if is_av1 {
                    ...
                    if std::fs::write(&stream_path, raw_slice).is_ok() {
                        for (fmt, codec_hint) in
                            [("av1", "libdav1d"), ("av1", "libaom-av1"), ("av1", "av1")]
                        {
...
pub fn extract_ffmpeg_frame_from_url(
    ...
    if !caps.supports_http {
        tg_log::warn(
            ...
            format!(
                "FFmpeg binary at '{}' does not support HTTP protocol",
                caps.path.display()
            ),
        );
        return None;
    ...
    if is_av1 && caps.av1_decoder.is_none() {
        tg_log::warn(
            ...
            format!(
                "FFmpeg binary at '{}' lacks AV1 decoder; falling back to smart file icon",
                caps.path.display()
            ),
        );
        return None;
    ...
    cmd.arg("-hide_banner")
        ...
        .arg("-hwaccel")
        .arg("none")
        // Force FFmpeg's HTTP input to use byte-range seeking instead of
        // treating the bridge as an endless sequential stream.
        .arg("-seekable")
        .arg("1")
        // The bridge intentionally closes short partial responses. A small
        // read deadline makes FFmpeg advance to the tail/moov range promptly
        // instead of waiting its default ~10 seconds before seeking.
        .arg("-rw_timeout")
        .arg("2000000")
        .arg("-probesize")
        .arg("1048576")
    ...
    // `tokio::time::timeout(spawn_blocking(...))` cannot cancel an OS child
    // after the blocking task has started. Enforce the deadline inside the
    // extractor so timed-out previews never leave orphaned FFmpeg processes.
    let started = Instant::now();
    let mut timed_out = false;
    loop {
        match child.try_wait() {
            Ok(Some(_)) => break,
            Ok(None) if started.elapsed() < Duration::from_secs(11) => {
            ...
            Err(e) => {
                let _ = child.kill();
                let _ = child.wait();
                tg_log::warn(BACKEND, "ffmpeg_url_wait_err", e.to_string());
                break;
            }
        }
    }
    ...
    if timed_out {
        tg_log::warn(
            BACKEND,
            "ffmpeg_url_extract_timeout",
            "deadline_ms=11000 child_killed=true",
        );
    }
...
mod tests {
    ...
    fn test_probe_capabilities() {
        if let Some(caps) = get_ffmpeg_capabilities() {
            ...
            assert!(
                caps.supports_http,
                "Selected FFmpeg binary MUST support HTTP protocol!"
            );
        } else {
