...
pub fn classify_media_category(file_name: &str, mime_type: Option<&str>) -> MediaCategory {
        ...
        || name_l.ends_with(".vob")
    {
        ...
        || name_l.ends_with(".3g2")
        || name_l.ends_with(".flv")
        || name_l.ends_with(".wmv")
        || name_l.ends_with(".ts")
        || name_l.ends_with(".asf")
        || name_l.ends_with(".rm")
        || name_l.ends_with(".rmvb")
        || name_l.ends_with(".divx")
        || name_l.ends_with(".f4v")
        || name_l.ends_with(".ogv")
    {
        MediaCategory::VideoConsumer
    } else if name_l.ends_with(".flac")
        || name_l.ends_with(".wav")
        || name_l.ends_with(".alac")
        || name_l.ends_with(".aiff")
        || name_l.ends_with(".aif")
        || name_l.ends_with(".ape")
        || name_l.ends_with(".dsd")
        || name_l.ends_with(".dsf")
        || name_l.ends_with(".dff")
    {
        MediaCategory::AudioLossless
    } else if mime_l.starts_with("audio/")
        || name_l.ends_with(".mp3")
        || name_l.ends_with(".m4a")
        || name_l.ends_with(".m4b")
        || name_l.ends_with(".aac")
        || name_l.ends_with(".ogg")
        || name_l.ends_with(".opus")
        || name_l.ends_with(".wma")
        || name_l.ends_with(".mid")
        || name_l.ends_with(".midi")
        || name_l.ends_with(".ac3")
        || name_l.ends_with(".eac3")
        || name_l.ends_with(".dts")
        || name_l.ends_with(".amr")
        || name_l.ends_with(".mka")
    {
        ...
        || name_l.ends_with(".gz")
        || name_l.ends_with(".tgz")
        || name_l.ends_with(".bz2")
        || name_l.ends_with(".tbz")
        || name_l.ends_with(".tbz2")
        || name_l.ends_with(".xz")
        || name_l.ends_with(".txz")
        || name_l.ends_with(".zst")
        || name_l.ends_with(".iso")
        || name_l.ends_with(".bin")
        || name_l.ends_with(".dmg")
        || name_l.ends_with(".apk")
...
pub fn classify_media_item(
    ...
    let is_video_format = mime_l.starts_with("video/")
        ...
        || name_l.ends_with(".divx")
        || name_l.ends_with(".f4v")
        || name_l.ends_with(".mpe")
        || name_l.ends_with(".mpeg")
        || name_l.ends_with(".mpg")
        || name_l.ends_with(".mpv")
        || name_l.ends_with(".mxf")
        || name_l.ends_with(".prores")
        || name_l.ends_with(".r3d")
        || name_l.ends_with(".braw");
    ...
    let is_archive_format = name_l.ends_with(".zip")
        ...
        || name_l.ends_with(".txz")
        || name_l.ends_with(".zst")
        || name_l.ends_with(".tzst")
        || name_l.ends_with(".iso")
        || name_l.ends_with(".cab")
        || name_l.ends_with(".dmg")
        || name_l.ends_with(".pkg")
        || name_l.ends_with(".deb")
        || name_l.ends_with(".rpm")
        || name_l.ends_with(".apk")
        || name_l.ends_with(".aab")
        || name_l.ends_with(".jar")
        || name_l.ends_with(".war")
        || name_l.ends_with(".ear")
        || name_l.ends_with(".cbz")
        || name_l.ends_with(".cbr")
        || name_l.ends_with(".cb7")
        || name_l.ends_with(".wim")
        || mime_l.contains("zip")
    ...
    let is_doc_format = name_l.ends_with(".pdf")
        ...
        || name_l.ends_with(".odt")
        || name_l.ends_with(".ods")
        || name_l.ends_with(".odp")
        || name_l.ends_with(".rtf")
        || name_l.ends_with(".epub")
        || name_l.ends_with(".mobi")
        || name_l.ends_with(".azw3")
        || name_l.ends_with(".txt")
        || name_l.ends_with(".csv")
        || name_l.ends_with(".tsv")
        || name_l.ends_with(".json")
        || name_l.ends_with(".xml")
        || name_l.ends_with(".yaml")
        || name_l.ends_with(".yml")
        || name_l.ends_with(".sql")
        || name_l.ends_with(".md")
        || name_l.ends_with(".markdown")
        || mime_l.contains("pdf")
